...
public class MisSistemasExactUI : MonoBehaviour
{
    ...
    [Tooltip("Arrastra aquí tu icono real del sistema solar.")]
    public Sprite spriteIconoSistema;
    [Tooltip("Arrastra un sprite blanco con bordes redondeados (Ej: 'UISprite' de Unity).")]
    ...
    private readonly string[] nombres = { "Sistema Alpha", "Galaxia Personal", "Nebulosa 42", "Constelación Nova", "Sistema Binario", "Experimento Orbital" };
    private readonly string[] infos = { "5 planetas", "8 planetas", "3 planetas", "12 planetas", "6 planetas", "4 planetas" };
    private readonly string[] fechas = { "10/3/2026", "12/3/2026", "13/3/2026", "14/3/2026", "15/3/2026", "15/3/2026" };
    ...
    public void LimpiarEscena()
    {
        ...
        for (int i = panelMisSistemas.childCount - 1; i >= 0; i--)
        {
            ...
            if (child.name.Contains("Auto")) DestroyImmediate(child);
        }
    }
    ...
    private void CrearTarjeta(Transform padre, string nombre, string info, string fecha)
    {
        ...
        if (spriteFondoTarjetaRedondeada != null)
        {
            ...
            bg.type = Image.Type.Sliced; // Crucial para que no se deforme al estirarse
            bg.pixelsPerUnitMultiplier = 1f; // Ajuste por defecto
        }

        // Borde exterior (Outline se adapta a la forma del sprite)
        ...
        CrearTextoEnTarjeta(tarjeta.transform, "Nombre", nombre, new Vector2(0.06f, 0.40f), new Vector2(0.94f, 0.55f), 24, FontStyles.Bold, Color.white, TextAlignmentOptions.Left);
        CrearTextoEnTarjeta(tarjeta.transform, "Info", info, new Vector2(0.06f, 0.28f), new Vector2(0.5f, 0.38f), 16, FontStyles.Normal, colorSubtitulo, TextAlignmentOptions.Left);
        CrearTextoEnTarjeta(tarjeta.transform, "Fecha", fecha, new Vector2(0.5f, 0.28f), new Vector2(0.94f, 0.38f), 15, FontStyles.Normal, colorSubtitulo, TextAlignmentOptions.Right);

        // --- BOTONES CON BORDES REDONDEADOS TAMBIÉN ---
    ...
    private void CrearIconoCentral(Transform padre)
    {
        ...
        // Lo anclamos en la parte superior central
        ...
        if (spriteIconoSistema != null)
        {
            ...
            img.color = Color.white; // Para que muestre los colores reales de tu imagen
            img.preserveAspect = true; // Para que no se achate
        }
        else
        {
            // Por si se te olvida ponerlo, pone un cuadro naranja para avisarte
    ...
    // ==========================================
    // MÉTODOS DE SOPORTE (RESTO DEL CÓDIGO)
    // ==========================================

    private void EstilizarPanelBase()
    {
        Image img = panelMisSistemas.GetComponent<Image>() ?? panelMisSistemas.gameObject.AddComponent<Image>();
        img.color = colorPanelFondo;
    }

    private void CrearBotonVolverNuevo()
    {
        if (botonVolverOriginal != null) botonVolverOriginal.gameObject.SetActive(false);

        GameObject btnNuevo = CrearObjetoUI("BtnVolverAuto", panelMisSistemas);
        RectTransform rt = btnNuevo.GetComponent<RectTransform>();
        rt.anchorMin = rt.anchorMax = new Vector2(0.05f, 0.95f);
        ...
        Outline outline = btnNuevo.AddComponent<Outline>();
        outline.effectColor = new Color32(60, 70, 110, 255);
        outline.effectDistance = new Vector2(1, -1);

        Button btn = btnNuevo.AddComponent<Button>();
        if (botonVolverOriginal != null) btn.onClick = botonVolverOriginal.onClick;
        ...
        if (fuenteTMP) tmp.font = fuenteTMP;
    }

    private void CrearCabeceraTexto()
    {
        CrearTextoTMP("TituloAuto", "Mis Sistemas Solares", new Vector2(0.05f, 0.88f), 46f, FontStyles.Bold, colorTitulo);
        CrearTextoTMP("SubtituloAuto", nombres.Length + " sistemas creados", new Vector2(0.05f, 0.82f), 18f, FontStyles.Normal, colorSubtitulo);
    }
    ...
    private void CrearTextoInterno(Transform padre, string texto, Color? color = null)
    {
        ...
        tmp.text = texto; tmp.fontSize = 14; tmp.alignment = TextAlignmentOptions.Center;
        tmp.color = color ?? Color.white;
        if (fuenteTMP) tmp.font = fuenteTMP;
    }

    private void CrearTextoEnTarjeta(Transform padre, string nombre, string contenido, Vector2 min, Vector2 max, float size, FontStyles estilo, Color color, TextAlignmentOptions alineacion)
    {
        GameObject go = CrearObjetoUI(nombre + "Auto", padre);
        ConfigurarAnclaje(go, min, max, Vector2.zero);
        var tmp = go.AddComponent<TextMeshProUGUI>();
        tmp.text = contenido; tmp.fontSize = size; tmp.fontStyle = estilo; tmp.color = color; tmp.alignment = alineacion;
        if (fuenteTMP) tmp.font = fuenteTMP;
    }

    private void CrearTextoTMP(string nombre, string contenido, Vector2 anclaje, float size, FontStyles estilo, Color color)
    {
        GameObject go = CrearObjetoUI(nombre, panelMisSistemas);
        RectTransform rt = go.GetComponent<RectTransform>();
        rt.anchorMin = rt.anchorMax = rt.pivot = anclaje;
        rt.anchoredPosition = Vector2.zero;
        rt.sizeDelta = new Vector2(800, 100);
        var tmp = go.AddComponent<TextMeshProUGUI>();
        tmp.text = contenido; tmp.fontSize = size; tmp.fontStyle = estilo; tmp.color = color;
